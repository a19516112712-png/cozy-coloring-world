...
function slugToTitle(slug) {
  return slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()).trim() + ' Coloring Page';
}

function cleanTitle(title, slug) {
  if (!title || title === slug || title.length < 10) return slugToTitle(slug);
  let t = title.trim();
  if (/ (Col|Co|Color|Colo|Colori|Colorin|C)$/i.test(t)) {
    t = t.replace(/ (Col|Co|Color|Colo|Colori|Colorin|C)$/i, '');
    if (!t.toLowerCase().includes('coloring page')) t += ' Coloring Page';
  }
  if (t.endsWith('C') && !t.endsWith(' Col')) t += 'oloring Page';
  else if (t.endsWith('Co')) t += 'loring Page';
  else if (t.endsWith('Col')) t += 'oring Page';
  else if (t.endsWith('Colo')) t += 'ring Page';
  else if (t.endsWith('Color')) t += 'ing Page';
  else if (t.endsWith('Colori')) t += 'ng Page';
  else if (t.endsWith('Colorin')) t += 'g Page';
  else if (t.endsWith(' Coloring') && !t.toLowerCase().endsWith('coloring page')) t += ' Page';
  else if (!t.toLowerCase().includes('coloring page') && !t.toLowerCase().includes('coloring pages')) t += ' Coloring Page';
  return t.charAt(0).toUpperCase() + t.slice(1);
}

function extractField(block, field) {
  // Try double quotes
  let m = block.match(new RegExp(field + ':\\s*"([^"]+)"'));
  if (m) return m[1];
  // Try single quotes (match until the closing single quote before a comma or newline)
  m = block.match(new RegExp(field + ":\\s*'([^']+)'"));
  if (m) return m[1];
  return '';
}
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!capturing && line.trim().startsWith('{') && i > 20) { block = line + '\n'; capturing = true; }
      ...
      if (line.trim().startsWith('},') || line.trim() === '},') {
        ...
        if (slug) entries.push({ slug, title, category });
...
// Fix titles
coloringPages.forEach(p => { p.title = cleanTitle(p.title, p.slug); });
categories.forEach(p => { p.title = cleanTitle(p.title, p.slug); if (!p.title.includes('Coloring')) p.title += ' Coloring Pages'; });
blogPosts.forEach(p => { p.title = cleanTitle(p.title, p.slug); });
...
function pinterestKeywords(title) {
  ...
  return base.split(' ').join(', ') + ', coloring page, free printable, cute coloring, JPG download, Tiny Animal Worlds';
}
...
coloringPages.forEach((p, i) => {
  pins.push({
    title: p.title,
    description: description(p.title, i),
    link: BASE_URL + '/coloring/' + p.slug,
    board: assignBoard(p.slug, p.category),
    keywords: pinterestKeywords(p.title),
    altText: altText(p.title),
    slug: p.slug,
  });
});
...
categories.forEach(c => {
  pins.push({
    title: c.title,
    description: 'Browse our ' + c.title.toLowerCase() + ' collection! Free printable JPG downloads for kids and adults.',
    link: BASE_URL + '/category/' + c.slug,
    board: assignBoard(c.slug, c.category || ''),
    keywords: c.title.toLowerCase() + ', coloring pages, free printable, category',
    altText: c.title + ' category page with free printable coloring pages from Tiny Animal Worlds.',
    slug: c.slug,
  });
});

blogPosts.slice(0, 10).forEach(b => {
  pins.push({
    title: b.title,
    description: b.title + ' — read our complete guide with tips, ideas, and free downloads!',
    link: BASE_URL + '/blog/' + b.slug,
    board: assignBoard(b.slug, b.category || ''),
    keywords: b.title.toLowerCase() + ', coloring blog, free printable, coloring tips',
    altText: 'Blog post: ' + b.title + ' on Tiny Animal Worlds coloring blog.',
    slug: b.slug,
  });
});
...
pins.slice(0, 20).forEach((p, i) => {
  ...
  console.log('   ' + p.description.substring(0, 95) + '...');
});
